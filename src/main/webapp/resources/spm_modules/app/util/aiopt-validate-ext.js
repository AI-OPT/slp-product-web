/*!
 * AIOPT 
 * 基于jQuery Validation Plugin v1.15.1
 *
...
* @param param:目标时间 单string，日期格式 yyyy-mm-dd 或者yyyy-mm-dd hh:mm:ss
...
$.validator.addMethod( "maxDate", function( value, element, param ) {
	...
	empty = $.trim(param).length?false:true;
	if(empty)return true;
	var d1 = new Date(value.replace("-","/"));  
	var d2 = new Date(param.replace("-","/"));				
	return Date.parse(d1)<=Date.parse(d2);	
}, $.validator.format( "日期不能大于 {0}." ) );
/**
* 校验某日期取值的最小值 add by gucl 20160828
* @param value:元素取值,单string，日期格式 yyyy-mm-dd 或者yyyy-mm-dd hh:mm:ss
* @param param:目标时间 单string，日期格式 yyyy-mm-dd 或者yyyy-mm-dd hh:mm:ss
* @return true/false:校验不通过为false
**/
...
$.validator.addMethod( "isDate", function( value, element, param ) {
	...
	return valid;
}, $.validator.format( "日期格式应为yyyy-MM-dd" ) );
...
* RETURNS: true/false
*/
$.validator.addMethod( "isDateTime", function( value, element, param ) {
	if(param==false)return true;
	/*如果参数值存在，则进行校验*/
	var empty = $.trim(value).length?false:true;
	if(empty)return true;
	...
	var valid =  (re.test(value))?true:false;		
	return valid;
}, $.validator.format( "日期时间格式应为yyyy-MM-dd HH:mm:ss" ) );
...
* 1个中文在ORACLE占2个字节
* @param value:元素取值
* @param param: ORACLE要求的最大长度
* @return true/false:校验不通过为false
**/
$.validator.addMethod( "cnlength", function( value, element, param ) {
	/*当前输入的长度*/
...
* @return true/false:校验不通过为false
**/
$.validator.addMethod( "notContainCN", function( value, element, param ) {
	if(param==false)return true;
	/*如果参数值存在，则进行校验*/
	var empty = $.trim(value).length?false:true;
	if(empty)return true;
...
* @return true/false:校验不通过为false
**/
$.validator.addMethod( "notContainSpace", function( value, element, param ) {
	if(param==false)return true;
	/*如果参数值存在，则进行校验*/
	var empty = $.trim(value).length?false:true;
	if(empty)return true;
...
/**
* 校验元素的取值是否为指定的值 add by gucl 2016-08-28
* @param value:元素取值,单string
* @param param:待比较的值
* @return true/false:校验不通过为false
**/
$.validator.addMethod( "unequalto", function( value, element, param ) {
	...
	var empty = $.trim(value).length?false:true;
	if(empty)return true;
	return $.trim(value)!=$.trim(param);
}, $.validator.format( "不能等于{0}" ) );
...
* @return true/false:校验不通过为false
**/
$.validator.addMethod( "moneyNumber", function( value, element, param ) {
	if(param==false)return true;
	/*如果参数值存在，则进行校验*/
	var empty = $.trim(value).length?false:true;
	if(empty)return true;
	...
	var valid =  (re.test(value))?true:false;
	return valid;
}, $.validator.format( "金额格式不对" ) );

/** add by gucl 2016-08-28
* 解决Number(8,2)类型数据的校验
* @param value:元素取值
* @param param: 有效整数位数和有效的小数位数（如: [8,2],8为有效位，2为小数位且可以为负数）
* @return true/false:校验不通过为false
**/
